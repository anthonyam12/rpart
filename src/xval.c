    ...
    */
    ...
    for (xgroup = 0; xgroup < n_xval; xgroup++) {
    	...
    	*/
    	...
    	(*rp_eval) (k, rp.ytemp, xtree->response_est, &(xtree->risk), rp.wtemp);    // anova SS
    	xtree->complexity = xtree->risk;
    	partition(1, xtree, &temp, 0, k);   //partition(int nodenum, pNode splitnode, double *sumrisk, int n1, int n2)
